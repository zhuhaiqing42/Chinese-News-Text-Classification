...
def open_file(filename, mode='r'):
    ...
    """
    return codecs.open(filename, mode, 'gbk')

def _savefile(savepath, label, content):
	fp = codecs.open(savepath, "a+",'gbk')
	fp.write(label+'\t')
	fp.write(content +'\n')
	fp.close()
...
if __name__ == '__main__':
	...
	#将训练集语料放入一个txt文件中
	file2txt('train_corpus_small/', 'thu_news/train.txt')
	
	#将测试集语料放入一个txt文件中
	file2txt('test_corpus_small/', 'thu_news/test.txt')

	#将验证集语料放入一个txt文件中
	file2txt('validation_corpus_small/', 'thu_news/validation.txt')
